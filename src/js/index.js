...
var getIM = function() {
  ...
  var ajaxGenIMList = function(res) {
    var imdata = JSON.parse(res);
    var imdataL = imdata.length;
    for (var i = 0; i < imdataL; i++) {
      var imDate = imdata[i].DateFormat;
          imDate = imDate.replace('yyyy', imdata[i].Year)
                         .replace('MM', imdata[i].Monthnum)
                         .replace('dd', imdata[i].Day)
                         .replace(' dddd', '<br/>' + imdata[i].Starttime + ' - ' + imdata[i].Endtime);
      var comDate = imDate.replace('yyyy', imdata[i].Year)
                          .replace('MM', imdata[i].Monthnum);
      if (imdata[i].Day < 10) {
        imdata[i].Day = "0" + imdata[i].Day;
      }
      comDate = comDate.replace('dd', imdata[i].Day)
                       .replace(' dddd', '<br/>' + imdata[i].Starttime + ' - ' + imdata[i].Endtime);
  ...
  var sortDateNappend = function(cityN, curIM) {
    Object.keys(cityList[cityN])
          .sort()
          .forEach(function(imNowD, i) {
  ...
  var newAjax = function(link, isLast) {
...
for (let i = 0; i < d_sliders.length; i++) {
  let tar = d_sliders[i];
  dest_sliders[i] = new fifi_slider({
    target: tar,
    autoHeight: true,
    dotsWrapper: $('#dots-wrapper-'+i)
  });
}
...
addListenerMulti(window, 'scroll DOMMouseScroll', function(e) {
  let bodyTop = (document.documentElement.scrollTop || document.body.scrollTop)+window.innerHeight/2;
  let testTop = $('.block-testimonial').offsetTop;
...
var animateBars = function() {
  ...
  for (let i = 0; i < statrows.length; i++) {
    let tarper = parseInt(statrows[i].getAttribute('data-num'));
    setTimeout(function() {
      statrows[i].querySelectorAll('.theBar')[0].style.width = tarper+"%";
      let cur = 0;
      let timer = setInterval(function() {
        if (cur < tarper) {
          statrows[i].querySelectorAll('.perNum')[0].innerHTML = cur+"%";
          cur++;
        } else {
          clearInterval(timer);
        }
...
function initMap() {
  ...
  offices = [
    {lat: 35.658769, lng: 139.705267},
    {lat: 43.064593, lng: 141.350434},
    {lat: 35.468254, lng: 139.621311},
    {lat: 35.168511, lng: 136.894026},
    {lat: 35.003964, lng: 135.757011},
    {lat: 34.69916, lng: 135.499127},
    {lat: 33.587739, lng: 130.3969}
  ];
...
function setCityonMap(idx) {
  ...
  let cities = document.querySelectorAll('.block-consultants .tab');
...
if (!hasClass($('body'), '.mkt-cn')) {
  ...
  script.onload = function() {
    ...
    addListenerMulti(window, 'scroll DOMMouseScroll', function(e) {
      let bodyTop = (document.documentElement.scrollTop || document.body.scrollTop)+window.innerHeight/2;
      let mapTop = $('.block-consultants').offsetTop;
